...
-- ── v3 Migration: Add session_id for anti-spam ────────────────────────────────
-- Run this if your orders table already exists:
alter table orders add column if not exists session_id text default null;

-- ── v3 Schema: orders table (full definition with all columns) ────────────────
-- If creating fresh, use this instead of the original:
/*
create table if not exists orders (
  id              uuid          primary key default gen_random_uuid(),
  order_id        text          not null,
  ...
  items           jsonb         not null default '[]',
  total           numeric(10,2) not null,   -- subtotal (before tax)
  tax             numeric(10,2) not null default 0,
  note            text          default '',
  instructions    text          default '',
  status          text          not null default 'pending'
