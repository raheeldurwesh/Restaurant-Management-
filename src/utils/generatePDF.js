...
export function generateInvoice(order, config = {}) {
  ...
  const taxPct         = Number(config.tax_percentage) || 8
  ...
  const C_QTY   = R - 34       // qty        — left
  const C_PRICE = R - 22       // unit price — left
  ...
  // ── Totals section ───────────────────────────────────────────────────────
  // order.total  = subtotal (items only, before tax)
  // order.tax    = tax amount
  // grand total  = order.total + order.tax
...
export function generateReport({ type, orders, label, config = {} }) {
  ...
  orders.forEach((order, idx) => {
    ...
    const itemStr = (order.items || []).map(i => `${i.qty}× ${i.name}`).join(', ')
