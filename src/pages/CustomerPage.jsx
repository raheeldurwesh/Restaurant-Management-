...
// FIXES:
//   1. customerName + instructions state — passed to placeOrder + CartDrawer
//   2. Dynamic categories from Firestore (not hardcoded array)
//   3. Mobile dropdown fix: full-width native <select> on all screen sizes
//   4. Tax is 100% dynamic from useConfig (no hardcoding)
//   5. Success screen shows customer name, items, subtotal/tax/total breakdown
...
export default function CustomerPage() {
  ...
  const taxPct = settings.taxPercentage  // always a valid number from useConfig
  ...
  // ── Dynamic categories from menu items (not hardcoded) ─────────────────────
  ...
  // ── Totals — always use live taxPct from config ─────────────────────────────
  ...
  const handlePlaceOrder = async () => {
    ...
    const id = await placeOrder({
      ...
      note,
      customerName,
      instructions: note,  // note doubles as instructions field
    })
    setLastOrder({
      orderId:      id,
      table:        tableNo,
        ...
        <div className="text-center max-w-sm animate-scale-in space-y-5 w-full">
          ...
          <div className="card px-5 py-4 text-left space-y-2.5">
            ...
            <div className="border-t border-border pt-2.5 space-y-1.5">
              <div className="flex justify-between text-sm font-body text-mid">
                <span>Subtotal</span>
                <span className="text-bright">₹{(lastOrder?.total || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm font-body text-mid">
              ...
              <div className="flex justify-between font-display font-bold border-t border-border pt-2">
                ...
                <span className="text-amber text-lg">₹{grandTotal.toFixed(2)}</span>
              </div>
            </div>
          ...
          <div className="card px-4 py-3 flex items-start gap-2.5 text-left">
            ...
            <p className="text-mid text-xs leading-relaxed">
              Please pay <strong className="text-bright">₹{grandTotal.toFixed(2)}</strong> at
    ...
    <div className="min-h-screen bg-base pb-28">
      ...
      <div className="sticky top-14 z-30 glass border-b border-border">
        <div className="max-w-5xl mx-auto px-4 py-3 space-y-3">
          ...
          {/* Search row — FIX: sort dropdown is full-width on mobile */}
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            />
            {/* MOBILE FIX: native select, full width on xs, auto on sm+ */}
            <select
              value={sort}
          ...
          {/* Category pills — dynamic from Firestore */}
